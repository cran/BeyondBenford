...
function(upbound,fig,dig=1){if (dig==1) 
	{if (fig>upbound) return(0) 
		...
		if (k>=2) {for (i in 0:(k-2)) {for (b in (fig*10**i):((fig+1)*10**i-1)) T=T+(b-v)*v/b**2; 
			for (a in ((fig+1)*10**i):(fig*10**(i+1)-1)) S=S+u*(a-u)/a**2; u=u+10**(i+1); v=v*10+8;}}; 
		if (upbound<((fig+1)*10**(k-1))) {for (b in (fig*10**(k-1)):upbound) T=T+(b-v)*v/b**2;} 
		else {for (b in (fig*10**(k-1)):((fig+1)*10**(k-1)-1)) T=T+(b-v)*v/b**2; for (a in ((fig+1)*10**(k-1)):upbound) S=S+u*(a-u)/a**2;}; 
	return(((S+T)/upbound**2)**0.5)}}
    ...
    if (k!=-1){
        for (i in 0:k) {
            for (j in (10**(dig-2)):(10**(dig-1)-1)) {
                for (b in ((10*j+fig)*10**i):((10*j+(fig+1))*10**i-1))
                    T=T+(b-((9*j+fig)*10**i+10**(dig-2)-1))*(b+1-10**(dig-1)-(b-((9*j+fig)*10**i+10**(dig-2)-1)))/(b+1-10**(dig-1))**2;}
		...
		if (max(10**(dig+i-1),(10*j+(fig+1))*10**i)<=min(10**(dig+i)-1,(10*(j+1)+fig)*10**i-1)) {
                for (a in max(10**(dig+i-1),(10*j+(fig+1))*10**i):min(10**(dig+i)-1,(10*(j+1)+fig)*10**i-1))
                    S=S+((j+1)*10**i-10**(dig-2))*((a+1-10**(dig-1))-((j+1)*10**i-10**(dig-2)))/(a+1-10**(dig-1))**2; }}}}
    if ((floor(upbound/10**(k+1))-10*floor(upbound/10**(k+2)))==fig & l>=10**(dig-2)) {
        for (j in 10**(dig-2):l) {
            for (b in ((10*j+fig)*10**(k+1)):min(upbound,(10*j+(fig+1))*10**(k+1)-1))
                T=T+(b-((9*j+fig)*10**(k+1)+10**(dig-2)-1))*((b+1-10**(dig-1))-(b-((9*j+fig)*10**(k+1)+10**(dig-2)-1)))/(b+1-10**(dig-1))**2;}
		...
		{for (a in max(10**(dig+k),(10*j+(fig+1))*10**(k+1)):((10*(j+1)+fig)*10**(k+1)-1))
                S=S+((j+1)*10**(k+1)-10**(dig-2))*((a+1-10**(dig-1))-((j+1)*10**(k+1)-10**(dig-2)))/(a+1-10**(dig-1))**2;}}}
    else {
		...
		if (max(10**(dig+k),(10*j+(fig+1))*10**(k+1))<=min(upbound,(10*(j+1)+fig)*10**(k+1)-1)) {
                ...
                S=S+((j+1)*10**(k+1)-10**(dig-2))*((a+1-10**(dig-1))-((j+1)*10**(k+1)-10**(dig-2)))/(a+1-10**(dig-1))**2;}}}}
    return(((S+T)/(upbound+1-10**(dig-1))**2)**0.5)}}}
